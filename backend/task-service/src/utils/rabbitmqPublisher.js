...
const ROUTING_KEY = process.env.ROUTING_KEY || "task.*";
...
async function retryConnect(fn, retries = 10, delayMs = 5000) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.error(
        `🔁 Retry ${i + 1}/${retries} - RabbitMQ belum siap: ${err.message}`
      );
      await delay(delayMs);
    }
  }
...
async function connectRabbitMQ() {
  try {
    return retryConnect(async () => {
      ...
      // 1. Buat Exchange (tipe topic)
      await channel.assertExchange(EXCHANGE_NAME, "topic", { durable: true });

      // 2. Buat Queue
      await channel.assertQueue(QUEUE_NAME, { durable: true });

      // 3. Bind Queue ke Exchange dengan routing key
      await channel.bindQueue(QUEUE_NAME, EXCHANGE_NAME, ROUTING_KEY);

      console.log("✅ RabbitMQ connected to Notification");
    });
  } catch (error) {
    console.error("❌ RabbitMQ connection notification error:", error);
  }
}
