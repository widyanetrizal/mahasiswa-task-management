...
} = require("./src/utils/progressStatusConsumer");
  ...
  try {
    ...
    while (retries) {
      try {
        ...
        retries--;
        console.log(
          `🔁 Database belum siap, coba lagi (${10 - retries}/10)...`
        );
        await waitFor(5000);
        if (retries === 0)
          throw new Error("❌ Gagal koneksi ke MySQL setelah 10x percobaan");
      }
    }
...
// app.listen(PORT, () => {
//   console.log(`Task Service berjalan di http://localhost:${PORT}`);
// });
